...
UNION ALL SELECT 'nulls_items_key', COUNT(*) FROM clean.order_items WHERE order_id IS NULL OR order_item_id IS NULL
UNION ALL SELECT 'nulls_payments_key', COUNT(*) FROM clean.payments WHERE order_id IS NULL
UNION ALL SELECT 'nulls_reviews_key', COUNT(*) FROM clean.reviews  WHERE review_id IS NULL OR order_id IS NULL;

/* ---------- 3) Dominios y rangos ---------- */
-- order_status permitido
SELECT 'bad_order_status', COUNT(*) FROM clean.orders
...
UNION ALL
-- payments: valores no negativos, installments >= 0
...
/* ---------- 6) FKs “trusted” (si las creaste con NOCHECK) ---------- */
